...
class Campaign:
    ...
    def __init__(self, campaign_dir):
        ...
        if "bedivfuzz" in self.id or "-ei-" in self.id or "random" in self.id or "-zest-mini-" in self.id:
            self.valid = False
        # if "chocopy" not in self.id:
        #     self.valid = False
        if not self.valid:
...
def get_corpus_sizes_at_time_limits(campaigns, min_executions_per_benchmark):
    ...
    print(min_executions_per_benchmark)
    for campaign in campaigns:
        benchmark = campaign.subject
        execution_limit = min_executions_per_benchmark.get(benchmark, 0)

        if execution_limit > 0:
            campaign.get_corpus_size_at_time_limit(execution_limit)
        else:
            print(f"\tSkipping corpus size calculation for campaign {campaign.id} due to zero execution limit")
    ...
    try:
        ...
        for _, row in corpus_df.iterrows():
            ...
            try:
                ...
                files_to_copy = all_files[:time_based_corpus_size]
